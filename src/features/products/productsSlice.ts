...
export const fetchProductsByName = createAsyncThunk<Product[], string, { state: RootState }>(
    "products/fetchProductsByName",
    async (name, { getState, rejectWithValue }) => {
        try {
            const token = getState().users.loggedUser?.token;
            const res = await axios.get(`/products/filterByName/options`, {
                headers: { Authorization: `Bearer ${token}` },
                params: { name }, // <---- aquí agregamos el query param
            });
            return res.data;
        } catch (error: any) {
            return rejectWithValue(error.response?.data || "Error cargando productos");
